    ...
    for (const path of paths) {
        ...
        for (const entry_path of glob_paths) {
            ...
            for (const entry of entries as ChromiumLocalStorage[]) {
                ...
                let key = entry.key === "" ? entry.origin : entry.key;
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                if (entries[i].origin.startsWith("META:") && entries[i].value_type === ValueType.Protobuf) {
                    // The first protobuf entry should be our timestamp
                    const time_value = (entries[i].value as Record<string, ProtoTag>)["1"];
                ...
                if (entries[i].value_type === ValueType.Protobuf) {
                    entries[i].value = JSON.stringify(entries[i].value);
                }
                // Update our entry
