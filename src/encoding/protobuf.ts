...
/**
 * A **very** minimal function to parse Protobuf data.
  ...
  while (proto_bytes.length != 0) {
    ...
    switch (tag.wire_type) {
      case WireType.Length: {
        ...
        //proto_values[tag.field] = value.message;
...
function extractStringOrMessage(
  ...
  // If its an error it should a be string then
...
function extractVarInt(raw_bytes: Uint8Array): Value | EncodingError {
  ...
  while (raw_bytes[byte_index] & 0x80) {
    var_bytes.push(raw_bytes[byte_index++] & 0x7f);
  }

  let var_value = 0;
  if (var_bytes.length === 0) {
    var_value = raw_bytes[0];
  } else {
    // get the last byte
    var_bytes.push(raw_bytes[byte_index++]);
    let i = var_bytes.length - 1;
    while (i >= 0) {
      var_value = (var_value << 7) + var_bytes[i--];
    }
  }
...
/**
 * Fucntion to parse a 64bit number
 * @param raw_bytes Protobuf bytes to parse
 * @returns A `Value` entry or `EncodingError`
 */
