    ...
    for (const path of paths) {
        ...
        for (const entry of results) {
            const history_row: FalkonCookie = {
                created: unixEpochToISO(webkitToIso(entry[ "creation_utc" ] as number)),
                ...
                expires: unixEpochToISO(webkitToIso(entry[ "expires_utc" ] as number)),
                is_secure: !!(entry[ "is_secure" ] as number),
                is_httponly: !!(entry[ "is_httponly" ] as number),
                last_access: unixEpochToISO(webkitToIso(entry[ "last_access_utc" ] as number)),
                ...
                last_update: unixEpochToISO(webkitToIso(entry[ "last_access_utc" ] as number)),
                ...
                evidence: full_path,
                version: path.version,
                datetime: unixEpochToISO(webkitToIso(entry[ "creation_utc" ] as number)),
