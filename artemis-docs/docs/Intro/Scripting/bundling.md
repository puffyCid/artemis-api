...
Currently artemis requires that we have all of our `JavaScript` code in one (1)
`.js` file. However, while very simple scripts may only be one (1) file, if we
decide to import an artemis function, or split our code into multiple files we
now have multiple files that need to be combine into one (1) `.js` file.
