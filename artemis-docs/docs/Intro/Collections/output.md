...
When you run artemis three files will be generated:

- `<artifact>_<uuid>.{output format}` a unique filename dependent on the format
  selected. These files contain the artifact data output. Depending on the
  collection multiple `<uuid>` files will be created
- `artemis_<uuid>.jsonl` a log file containing any errors or warnings generated by artemis
...
The `artemis_<uuid>.jsonl` output from a collection contains any errors or warnings
...
```
artemis acquire --format timeline prefetch
```
